...
def load_to_sql(file_path):
    ...
    engine = sqlalchemy.create_engine(f"postgresql+psycopg2://{conn.login}:{conn.password}@mlops-project-3_cf880f-postgres-1:{conn.port}/{conn.schema}")
...
with DAG(
    ...
    list_files = GCSListObjectsOperator(
        task_id="list_files",
        bucket="academic-atlas-475811-c5", 
    )
